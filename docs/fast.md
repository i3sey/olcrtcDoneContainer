...
Серверный и клиентский скрипты делают `podman pull` готового образа из GHCR.
Сборка и публикация контейнера происходят в GitHub Actions.
...
По умолчанию скрипт тянет `ghcr.io/i3sey/olcrtc:latest`.
Если нужно подменить образ, задай `OLCRTC_IMAGE` или передай `--image=...`.
